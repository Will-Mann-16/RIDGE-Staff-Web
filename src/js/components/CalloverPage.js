...
class CalloverPage extends React.Component {
  ...
  changeActiveCallover(index) {
    this.setState({
      ...this.state,
      activeCallover: this.props.callover.callovers[index]
    });
  }
  ...
  componentWillMount() {
    ...
    this.props.locations.locations.forEach(location => {
      if (
        location._id === this.props.user.config.DEFAULT_LOCATION &&
        defaultLocation !== null
      ) {
  ...
  render() {
    var studentHTML = this.props.students.students.map((student, key) => {
      return (
        <tr key={key}>
          ...
          <td
            style={{ backgroundColor: student.location.colour, color: 'white' }}
          >
          ...
          >
            ...
            >
              {this.state.calledOver.indexOf(student._id) === -1 ? (
                <button
                  className="btn-green btn-icon-small"
                  style={{ width: '100%', height: '100%' }}
                  onClick={this.callOver.bind(this, student._id)}
                >
                ...
                <button
                  ...
                  style={{ width: '100%', height: '100%' }}
                  onClick={this.removeCallOver.bind(this, student._id)}
                >
    ...
    var calloverHTML = this.props.callover.callovers.map((callover, key) => {
      ...
      return (
        <li
          key={key}
          onClick={this.changeActiveCallover.bind(
            this,
            this.props.callover.callovers.indexOf(callover)
          )}
          className={this.state.activeCallover === callover ? 'active' : null}
        >
    ...
    return (
      <div className="container">
        <div className="icon-bar">
          <a
            className={this.state.newCallover ? 'active' : ''}
            onClick={this.addCallover.bind(this)}
          ...
          <a
            className={!this.state.newCallover ? 'active' : ''}
            onClick={this.viewCallover.bind(this)}
            style={{ textDecoration: 'none', color: 'white' }}
          >
        ...
        {this.state.newCallover ? (
          <div>
            ...
            <button
              ...
              onClick={this.submitCallover.bind(this)}
            >
              Submit
            </button>
          </div>
          ...
          <div className="row">
            <div className="col-3">
              <ul className="list">{calloverHTML}</ul>
            </div>
            <div className="col-9">{renderHTML(this.state.activeCallover)}</div>
          </div>
        )}
      </div>
    );
