...
class StudentListPage extends React.Component {
    ...
    componentWillMount() {
        ...
        this.setState({...this.state, filters: {...this.state.filters, yeargroup: yeargroupFilterList}});
    }
    ...
    render() {
        ...
        const studentHTML = this.props.students.students.map((student, key) => {
            ...
            var display = (search.test(student.firstname) || search.test(student.surname) || search.test(student.code) || search.test(this.props.user.config.YEARGROUP_NAMES[student.yeargroup]) || search.test(student.location) || search.test(timeString)) && this.state.filters.yeargroup[student.yeargroup] && this.state.filters.blackLocations.indexOf(student.location.id) == -1;
            return display ? (<tr key={key}>
                ...
                <td style={{textAlign: 'center'}}><Link to={link} style={{textDecoration: "none", color: "black"}}><i
                    className="fa fa-edit"></i></Link></td>
                        ...
                        {({on, toggle}) => {
                            return on ? (
                                <div className="btn-group"
                                    ...
                                    <button className="btn-green btn-icon-small" style={{width: "50%", height: "100%"}}
                                            onClick={this.delete.bind(this, student._id)}><i className="fa fa-trash"></i>
                                    </button>
                                    <button className="btn-red btn-icon-small" style={{width: "50%", height: "100%"}}
                                            onClick={toggle}><i className="fa fa-times"></i></button>
                                </div>
                            ) : (
                                    ...
                                    return (
                                        <div>
                                            <div className="icon-bar">
                                                ...
                                                <a onClick={uploadToggle}>Upload <i className="fa fa-cloud-upload"></i></a>
                                                <a onClick={filterToggle}>Filter <i className="fa fa-filter"></i></a>
                                            </div>
                                            ...
                                            {filterOn && <div className="row">
                                                <input className="col-4 input" name="search" placeholder="Search" onChange={(e) => this.setState({...this.state, filters: {...this.state.filters, search: e.target.value}})}/>
                                                <div className="col-4">
                                                    {this.props.user.config.YEARGROUP_NAMES.map((name, key) =>
                                                    <label className="checkbox">{name}
                                                        <input type="checkbox" onChange={this.toggleYeargroup.bind(this, key)}
                                                               checked={this.state.filters.yeargroup[key] ? "checked" : null}/>
