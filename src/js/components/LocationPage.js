...
class LocationPage extends React.Component{
  ...
  componentWillMount(){
    if(this.props.edit){
      ...
      this.setState({...this.state, location:{...this.state.location, _house: this.props.user.user.user.house}});
    }
  }
  handleChange(event) {
  ...
  submitData(){
    if(this.props.edit){
      this.props.dispatch(updateLocation(this.state.location._id, this.state.location, this.props.user.user.user.house));
    }else{
      this.props.dispatch(createLocation({...this.state.location, order: this.props.locations.locations.length + 1}, this.props.user.user.user.house));
    }
    ...
    if(this.state && this.state.submitted){
      ...
      return(
        <div className="container row">
          <h3>Edit Location</h3>
          <div className="col-6">
            <input required className="form-input" onChange={this.handleChange.bind(this)} name="name" value={this.state.location.name} placeholder="Name"/>
            ...
            <select required className="form-input" name="heading" onChange={this.handleChange.bind(this)}>
                {this.props.user.config.LOCATION_HEADINGS.map((heading, key) => {
                    return (<option key={key} value={key}>{heading}</option>);
                })}
            </select>
          ...
          {this.props.edit ? (<div className="col-6">
