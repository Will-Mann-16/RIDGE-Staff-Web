    ...
    studentHTML = this.props.students.students.map((student, key) =>{
      ...
      locationHTML = locations.map((locationsList, key) => {
        return locationsList.map((location, key2) => {
            return (<LocationButton location={location} break={key2 == 0} headingName={this.props.user.config.LOCATION_HEADINGS[location.heading]} key={key2} updateLocation={this.updateLocation.bind(this)}/>);
        })
      });
      const locationToggleHTML =  (      <div className="col-2">  <Toggle initial={false}>
          {({on, toggle}) => {return (<div><div className="accordian location-button" onClick={toggle}>
            <div className="location-button-body">Select</div>
          </div>{on && (<div className="panel">
            <button className="select-button" onClick={this.deselectAll.bind(this)}>Deselect All</button>
              {this.props.user.config.YEARGROUP_NAMES.map((yeargroup, key) => <button className="select-button" onClick={this.selectYear.bind(this, key)}>{yeargroup}</button>)}
          </div>)}</div>)}}</Toggle>{locationHTML}</div>);
    return(
      <div id="view-grid" className="row" style={{marginTop: this.props.view ? 0 : 50}}>
        <div className={this.props.students.selected.length > 0 ? "col-10" : ""}>
        {studentHTML}
      </div>
        {this.props.students.selected.length > 0 && locationToggleHTML}
    </div>
  );
  }
}
