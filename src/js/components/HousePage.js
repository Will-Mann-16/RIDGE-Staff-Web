    ...
    render(){
        ...
        return(
            <div className="container">
                ...
                <input className="form-input" value={this.state.house.name} onChange={this.handleNameChange.bind(this)} placeholder="Name"/>
                <Toggle>
                    {({on,toggle}) =>
                        <div>
                        <div style={{
                            ...
                            {this.state.house.personell.map((person, key) => (
                                <p className="btn-black-onyx" onClick={this.removePerson.bind(this, person)}
                                   style={{flexGrow: 1, flexBasis: "100%"}} key={key}>{person}</p>))}
                            <button className="btn-blue" onClick={toggle} style={{
                                flexGrow: 1,
                                flexBasis: "100%"
                            }}>{on ? "Close" : "Add Person"}</button>
                        </div>
                        {
                            on ?
                                <div className="row" style={{marginTop: 5}}><input style={{float: "left", width: "75%"}}
                                    ...
                                    <button className="btn-green" style={{float: "right", width: "25%"}}
                                            onClick={this.addPersonToList.bind(this)}>Add
                                    </button>
                                </div> : null
                        }
                        </div>
                        ...
                        {({on, toggle}) => {
                            ...
                            <div style={{
                                ...
                                {this.state.house.colours.map((colour, key) => (
                                    <p onClick={this.removeColour.bind(this, colour)} className="btn-black-onyx"
                                       style={{backgroundColor: colour, flexGrow: 1, flexBasis: "100%"}}
                                       key={key}></p>))}
                                <button className="btn-blue" onClick={toggle} style={{
                                    flexGrow: 1,
                                    flexBasis: "100%"
                                }}>{on ? "Close" : "Add Colour"}</button>
                            </div>
                            {
                                on ? <div className="row" style={{marginTop: 5}}>
                                    <div style={{margin: "auto", maxWidth: 300}}><SketchPicker width={300} disableAlpha
                                                                                               color={this.state.newColour}
                                                                                               onChangeComplete={this.handleChangeColour.bind(this)}/>
                                        <button className="btn-green" style={{width: "100%"}}
                                                onClick={this.addColourToList.bind(this)}>Add
                                        </button>
                                    </div>
                                </div> : null
                            }
                ...
                <button className="btn-green" style={{marginTop: 5}} onClick={this.addHouse.bind(this)}>{this.props.edit ? "Update" : "Create"}</button>
            </div>
        );
    }
}
