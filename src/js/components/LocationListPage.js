...
class LocationListPage extends React.Component{
    ...
    changeOrderUp(location, list){
        if(list.indexOf(location) > 0){
            ...
            this.props.dispatch(updateLocation(aboveLoc._id, aboveLoc, this.props.user.user.user.house));
            this.props.dispatch(updateLocation(location._id, location, this.props.user.user.user.house));
        }
    }
    changeOrderDown(location, list){
        if(list.indexOf(location) < list.length - 1){
            ...
            location.order = order;
            this.props.dispatch(updateLocation(belowLoc._id, belowLoc, this.props.user.user.user.house));
            this.props.dispatch(updateLocation(location._id, location, this.props.user.user.user.house));
        }
    }
    render(){
        ...
        const LocationRow = ({location, list}) => {
            ...
            var display = (search.test(location.name) || search.test(location.colour) || search.test(location.heading));
            return display ? (<tr>
                <td>{location.name}</td>
                <td>{headingHTML()}</td>
                    ...
                    <Toggle initial={false}>
                        {({on, toggle}) => {return on ? (
                            <div className="btn-group" style={{position: "absolute", top: 0, bottom: 0, right: 0, left: 0}}>
                                <button className="btn-green btn-icon-small" style={{width: "50%", height: "100%"}} onClick={this.delete.bind(this, location._id)}><i className="fa fa-trash"></i></button>
                ...
                <td style={{textAlign: 'center', height: "100%", position: "relative"}}><div className="btn-group" style={{position: "absolute", top: 0, bottom: 0, right: 0, left: 0}}><button className="btn-african-sapphire btn-icon-small" onClick={this.changeOrderUp.bind(this, location, list)} style={{width: "50%", height: "100%"}}><i className="fa fa-arrow-circle-o-up"></i></button><button className="btn-african-sapphire btn-icon-small"  onClick={this.changeOrderDown.bind(this, location, list)} style={{width: "50%", height: "100%"}}><i className="fa fa-arrow-circle-o-down"></i></button></div></td>
            </tr>) : null;
        };
        var locations = [];
                ...
                <Toggle initial={false}>
                    {({on, toggle}) =>
                        ...
                        <div className="icon-bar">
                            <Link to="/locations/new" style={{textDecoration: "none", color: "white"}}>Add <i
                                className="fa fa-plus-square"></i></Link>
                            <a onClick={toggle}>Filter <i className="fa fa-filter"></i></a>
                        </div>
                        {on && <div>
                            <input className="form-input" name="search" placeholder="Search" onChange={(e) => this.setState({...this.state, filters: {...this.state.filters, search: e.target.value}})}/>
                        </div>}
                        </div>
                    }
                </Toggle>
