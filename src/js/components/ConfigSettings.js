...
class ConfigSettings extends React.Component{
    ...
    componentWillMount(){
               ...
               this.setState({...this.state, defaultLocation: {id: this.props.user.config.DEFAULT_LOCATION, location: location}, yeargroups: yeargroups, locationHeadings: this.props.user.config.LOCATION_HEADINGS})
    }
    ...
    removeLocationHeading(locationHeading){
        ...
        this.setState({...this.state, submittable: true, locationHeadings: locationHeadings});
    }
    ...
    saveConfig(){
        ...
        var result = {DEFAULT_LOCATION: defaultLocation, YEARGROUP_NAMES: yeargroups, LOCATION_HEADINGS: locationHeadings};
        if(this.state.submittable){
            this.props.dispatch(modifyHouseConfig(this.props.user.user.user.house, result));
        }
    }
        ...
        return(
            <div className="container">
                <div className="config-option">
                    ...
                    <select required className="form-input" name="conf-default-location" style={{backgroundColor: this.state.defaultLocation.location.colour, borderColor: this.state.submittable ? "black" : "red"}} onChange={this.handleDefaultLocationChange.bind(this)}>
                        {locationHTML}
                    </select>
                </div>
                <Toggle>
                        ...
                        <div className="config-option">
                            <div style={{
                                ...
                                {this.state.yeargroups.map((person, key) => (
                                    <p className="btn-black-onyx" onClick={this.removeYeargroup.bind(this, person)}
                                       style={{flexGrow: 1, flexBasis: "100%"}} key={key}>{person}</p>))}
                                <button className="btn-blue" onClick={toggle} style={{
                                    flexGrow: 1,
                                    flexBasis: "100%"
                                }}>{on ? "Close" : "Add Yeargroup"}</button>
                            </div>
                            {
                                on ?
                                    <div className="row" style={{marginTop: 5}}><input style={{float: "left", width: "75%"}}
                                                                                   className="input" type="text"
                                                                                   placeholder="New Yeargroup"
                                                                                   onChange={this.updateYeargroup.bind(this)}/>
                                        <button className="btn-green" style={{float: "right", width: "25%"}}
                                                onClick={this.addYeargroupToList.bind(this)}>Add
                                        </button>
                                    </div> : null
                            }
                        </div>
                        ...
                        <div className="config-option">
                            <div style={{
                                ...
                                {this.state.locationHeadings.map((locationHeading, key) => (
                                    <p className="btn-black-onyx" onClick={this.removeLocationHeading.bind(this, locationHeading)}
                                       style={{flexGrow: 1, flexBasis: "100%"}} key={key}>{locationHeading}</p>))}
                                <button className="btn-blue" onClick={toggle} style={{
                                    flexGrow: 1,
                                    flexBasis: "100%"
                                }}>{on ? "Close" : "Add Location Heading"}</button>
                            </div>
                            {
                                on ?
                                    <div className="row" style={{marginTop: 5}}><input style={{float: "left", width: "75%"}}
                                        ...
                                        <button className="btn-green" style={{float: "right", width: "25%"}}
                                                onClick={this.addLocationHeadingToList.bind(this)}>Add
                                        </button>
                                    </div> : null
                            }
                        </div>
                ...
                <button className="btn-green" onClick={this.saveConfig.bind(this)}>Save</button>
            </div>
        );
    }
}
