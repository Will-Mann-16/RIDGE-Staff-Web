...
class HistoryListPage extends React.Component{
  ...
  pullData(){
    axiosToken.get("history/read", {params: {house: this.props.user.user.user.house, amount: this.state.amount, filter: this.state.search}}).then((response) =>{
  ...
  render(){
    var historyHTML = this.state.history.map((history, key) => {
        ...
        return(<tr key={key}>
          ...
          <td>{this.props.user.config.YEARGROUP_NAMES[history.student.yeargroup]}</td>
    ...
    return(
      <div className="container">
        <div className="icon-bar">
          <input className="form-input" placeholder="Search..." style={{width: "100%", margin: 5}} onChange={this.handleChange.bind(this)} name="search" />
        </div>
        ...
        <button onClick={this.readMore.bind(this)} className="btn-blue" style={{width: "100%"}}>Read More</button>
      </div>
    );
  }
}
