...
export function createCalender(event, house) {
  return dispatch => {
    ...
    axiosToken
      .post('calender/create', { house: house, event: event })
      .then(response => {
        if (response.data.success) {
          dispatch(readCalender());
          dispatch({
            type: 'CREATE_CALENDER_FULFILLED',
            payload: response.data.success
          });
        } else {
          dispatch({
            type: 'CREATE_CALENDER_REJECTED',
...
export function readCalender(house) {
  return dispatch => {
    ...
    axiosToken
      .get('calender/read', { params: { house: house } })
      .then(response => {
        if (response.status === 200 && response.data.success) {
          dispatch({
            type: 'READ_CALENDER_FULFILLED',
            payload: response.data.events
          });
        } else {
          dispatch({
            type: 'READ_CALENDER_REJECTED',
            payload: response.data.reason
          });
        }
      })
...
export function updateCalender(id, event, house) {
  return dispatch => {
    ...
    axiosToken
      .post('calender/update', { id: id, event: event, house: house })
      .then(response => {
        if (response.status === 200 && response.data.success) {
          dispatch(readCalender());
          dispatch({
            type: 'UPDATE_CALENDER_FULFILLED',
            payload: response.data.event
          });
        } else {
          dispatch({
            type: 'UPDATE_CALENDER_REJECTED',
            payload: response.data.reason
          });
        }
      })
...
export function deleteCalender(id, house) {
  return dispatch => {
    ...
    axiosToken
      .get('calender/delete', { params: { id: id, house } })
      .then(response => {
        if (response.status === 200 && response.data.success) {
          dispatch(readCalender());
          dispatch({
            type: 'DELETE_CALENDER_FULFILLED',
            payload: response.data.success
          });
        } else {
          dispatch({
            type: 'DELETE_CALENDER_REJECTED',
            payload: response.data.reason
          });
        }
      })
