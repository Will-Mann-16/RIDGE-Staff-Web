...
export function createUser(user, house){
  return dispatch => {
    ...
    axiosToken.post("users/create", { user: user, house: house }).then((response) =>{
...
export function readUser(){
  return dispatch => {
    ...
    axiosOpen.post("users/read", { jwt: localStorage.getItem("AUTH-TOKEN") ? localStorage.getItem("AUTH-TOKEN") : false }).then((response) =>{
      if(response.status === 200 && response.data.success){
        dispatch(setActiveConfig(response.data.user.user.house));
        dispatch({type: "READ_USER_FULFILLED", payload: {user: response.data.user, config: response.data.config}});
      }
...
export function updateUser(id, user, house){
  return dispatch => {
    ...
    axiosToken.post("users/update", { id: id, user: user, house: house }).then((response) =>{
      if(response.status === 200 && response.data.success){
...
export function deleteUser(id, house){
  return dispatch => {
    ...
    axiosToken.get("users/delete",  {params: { id: id, house: house}}).then((response) =>{
      if(response.status === 200 && response.data.success){
...
export function authenticateUser(username, password){
  return dispatch => {
    ...
    axiosOpen.post("users/authenticate", {username: username, password: password}).then((response) =>{
...
export function setActiveConfig(house){
    return dispatch => {
        ...
        axiosToken.get("users/read-config", { params: {house: house} }).then((response) =>{
            if(response.status === 200 && response.data.success){
                dispatch({type: "SET_CONFIG_FULFILLED", payload: response.data.config});
            }
            else{
                dispatch({type: "SET_CONFIG_REJECTED", payload: response.data.reason});
            }
        }).catch((err) =>{
