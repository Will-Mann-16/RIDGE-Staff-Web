...
export function createHouse(house) {
  return dispatch => {
    ...
    axiosToken
      .post('houses/create', { house: house })
      .then(response => {
...
export function readHouses(house) {
  return dispatch => {
    ...
    axiosToken
      .get('houses/read', { params: { house: house } })
      .then(response => {
...
export function updateHouse(id, house) {
  return dispatch => {
    ...
    axiosToken
      .post('houses/update', { id: id, house: house })
      .then(response => {
        if (response.status === 200 && response.data.success) {
...
export function deleteHouse(id) {
  return dispatch => {
    ...
    axiosToken
      .get('houses/delete', { params: { id: id } })
      .then(response => {
        if (response.status === 200 && response.data.success) {
          dispatch(readHouses());
          dispatch({
...
export function modifyHouseConfig(house, config) {
  return dispatch => {
    ...
    axiosToken
      .post('houses/update-config', { house: house, config: config })
      .then(response => {
        if (response.status === 200 && response.data.success) {
