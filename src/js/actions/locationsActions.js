...
export function createLocation(location, house){
  return dispatch => {
    ...
    axiosToken.post("locations/create", { location: location, house: house }).then((response) =>{
...
export function readLocations(house){
  return dispatch => {
    ...
    axiosToken.get("locations/read", {params: { house: house }}).then((response) =>{
      if(response.status === 200 && response.data.success){
...
export function updateLocation(id, location, house){
  return dispatch => {
    ...
    axiosToken.post("locations/update",  { id: id, location: location, house: house }).then((response) =>{
      if(response.status === 200 && response.data.success){
...
export function deleteLocation(id, house){
  return dispatch => {
    ...
    axiosToken.get("locations/delete",  {params: { id: id, house: house }}).then((response) =>{
      if(response.status === 200 && response.data.success){
          emit("socket-client-server-redraw-major");
